...
def main():
    ...
    # Convert timedeltas to human-readable hours
    results['cumulative_hours'] = results['cumulative_duration'].dt.total_seconds() / 3600.0
    ...
    # Display the results
    print("\n" + "="*70)
    print(f"{'Oblast (Region)':<30} | {'Alerts':<6} | {'Union (Hrs)':<12} | {'Cumulative (Hrs)':<16}")
    print("="*70)
    for _, row in results.iterrows():
        print(f"{row['oblast']:<30} | {row['alert_count']:<6} | {row['union_hours']:11.2f} | {row['cumulative_hours']:15.2f}")
    print("="*70)
    ...
    with open(summary_file, 'w') as f:
        ...
        f.write(f"{'Oblast':<30} | {'Alerts':<6} | {'Union (Hrs)':<12} | {'Cumulative (Hrs)':<16}\n")
        f.write("-" * 75 + "\n")
        for _, row in results.iterrows():
            f.write(f"{row['oblast']:<30} | {row['alert_count']:<6} | {row['union_hours']:11.2f} | {row['cumulative_hours']:15.2f}\n")
